...
Процедура ОписаниеКоманды(КомандаПриложения) Экспорт
	...
	КомандаПриложения.Опция("U ib-user", "", "пользователь информационной базы для подготовки обновления")
		.ТСтрока()
		.ВОкружении("RECTOR_IB_USER");

	КомандаПриложения.Опция("P ib-pwd", "", "пароль пользователя информационной базы для подготовки обновления")
		.ТСтрока()
...
Процедура ВыгрузитьКонфигурацию(Знач Путь, Знач ПараметрыКонфигурации)
	...
	ИначеЕсли ПараметрыКонфигурации.ТипИсходников = ТипыИсходников().Хранилище Тогда
		...
		КонфигураторВременнойБазы.УстановитьКонтекст(ПараметрыОбновления.ВременнаяБаза,
		                                             ПараметрыОбновления.ВременнаяБазаПользователь,
		                                             ПараметрыОбновления.ВременнаяБазаПароль);
...
Процедура ИнициализацияПроекта(КомандаПриложения)
	...
	ПараметрыОбновления.ВременнаяБазаПользователь = КомандаПриложения.ЗначениеОпции("ib-user");
	ПараметрыОбновления.ВременнаяБазаПароль       = КомандаПриложения.ЗначениеОпции("ib-pwd");
...
Функция СтруктураПараметровОбновления()
	...
	СтруктураПараметров.Вставить("ВременнаяБазаПользователь", "");
	СтруктураПараметров.Вставить("ВременнаяБазаПароль"      , "");
